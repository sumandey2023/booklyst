...
const ResturantComponent = () => {
  ...
  return (
    <MotionFade>
      <div
        onClick={handleClick}
        className="group cursor-pointer bg-gradient-to-br from-yellow-100 via-white to-yellow-50 dark:from-slate-800/40 dark:via-slate-900/40 dark:to-slate-800/40 hover:from-yellow-200 hover:to-white transition-all duration-300 shadow-xl hover:shadow-2xl rounded-3xl p-6 h-64 flex flex-col justify-center items-center border border-yellow-100 dark:border-slate-800 hover:-translate-y-2 focus:ring-2 focus:ring-yellow-400 outline-none"
        tabIndex={0}
        role="button"
        aria-label="Go to Restaurant"
      >
        <div className="flex items-center justify-center w-16 h-16 rounded-full bg-yellow-200 dark:bg-slate-800 mb-4 shadow group-hover:scale-110 transition-transform">
          <BuildingStorefrontIcon className="w-8 h-8 text-yellow-700 dark:text-yellow-300" />
        </div>
        <h2 className="text-2xl font-bold text-yellow-800 dark:text-yellow-300 mb-2">Restaurant</h2>
